...
if __name__ == "__main__":
    ...
    n_tasks = len(policy_names)
    print(f"Using {n_tasks} tasks")
    n_eps = 0
    pn = np.array(policy_names)
    while n_eps < EPISODES:
        SEED += 1
        obs, _info = env.reset(seed=SEED)
        writer.start_episode()
        latch = np.zeros(n_tasks, dtype=np.bool)
        for step in tqdm(range(MAX_STEPS)):
            ...
            s = _info.get("success", None)
            if s is None:
                s = _info["final_info"]["success"]
            latch |= (s == 1)
            if np.all(latch):
                n_eps += 1
            ...
            print(f"Exhausted all steps. {pn[np.where(~latch)[0]]} tasks failed. Trying again with {SEED}...")
