...
class FusionTransformer(nn.Module):
    def __init__(self, cfg: VLAConfig, device: torch.device):
        ...
        # If you get the excess weights error, do the embedding swap
    ...
    def _self_attn_mask(self, pad_mask: torch.Tensor, dtype: torch.dtype) -> torch.Tensor:
        # Convert bool pad mask (True=padding) to additive -inf mask for SDPA
        mask = torch.zeros(pad_mask.shape[0], 1, 1, pad_mask.shape[1], dtype=dtype, device=pad_mask.device)
        mask.masked_fill_(pad_mask[:, None, None, :], float('-inf'))
        return mask
    ...
    def forward(self, img: torch.Tensor, txt: torch.Tensor, state: torch.Tensor,
        ...
        self_attn_mask = (
            self._self_attn_mask(txt_pad_mask, txt.dtype)
            if txt_pad_mask is not None else None
        )

        x = txt
        for layer in self.layers:
            x = layer(x, kv, self_attn_mask=self_attn_mask)
